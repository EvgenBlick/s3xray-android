...
class XrayVPNService : VpnService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (command == AppConfigs.V2RAY_SERVICE_COMMANDS.START_SERVICE ||
            ...
            if (config != null) {
                // Ensure clean state before starting or switching
                cleanup()
                if (command == AppConfigs.V2RAY_SERVICE_COMMANDS.RESTART_SERVICE) {
                    XrayCoreManager.stopCore(this, notifyDisconnected = false)
                }
    ...
    private fun stopTun2socksProcess() {
        val process = tun2socksProcess ?: return
