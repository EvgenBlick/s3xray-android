...
class FlutterVlessPlugin : FlutterPlugin, ActivityAware, PluginRegistry.ActivityResultListener, MethodChannel.MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            "startVless" -> {
                ...
                // 5. Start or restart the XrayVPNService
                // Use a restart command to let the service switch tunnels atomically
                val intent = Intent(context, XrayVPNService::class.java)
                intent.putExtra("COMMAND", AppConfigs.V2RAY_SERVICE_COMMANDS.RESTART_SERVICE)
    ...
    private fun registerReceiver() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(xrayReceiver, filter, Context.RECEIVER_EXPORTED)
        } else {
            context.registerReceiver(xrayReceiver, filter)
        }
        receiverRegistered = true
    }

    private fun unregisterReceiver() {
        if (receiverRegistered && xrayReceiver != null) {
            try {
