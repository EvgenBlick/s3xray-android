...
object XrayCoreManager {
    ...
    @Volatile
    private var trafficQueryInFlight = false
    @Volatile
    private var lastTrafficSnapshot = longArrayOf(0L, 0L, 0L, 0L)
    @Volatile
    private var activeProcessGeneration = 0
    ...
    fun startCore(context: Service, config: XrayConfig): Boolean {
        stopCore(context, notifyDisconnected = false)
        ...
        try {
            ...
            xrayProcess = process
            val processGeneration = activeProcessGeneration + 1
            ...
            Thread {
                try {
                    ...
                    if (activeProcessGeneration == processGeneration &&
                        AppConfigs.V2RAY_STATE == AppConfigs.V2RAY_STATES.V2RAY_CONNECTED
                    ) {
                        // Unexpected exit
                        stopCore(context)
     ...
     */
    fun stopCore(context: Service, notifyDisconnected: Boolean = true) {
        try {
    ...
    private fun startTimer(context: Context) {
        ...
        countDownTimer = object : CountDownTimer(Long.MAX_VALUE, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                ...
                startTrafficQuery(context.applicationContext)
                val traffic = lastTrafficSnapshot
    ...
    private fun startTrafficQuery(context: Context) {
        if (trafficQueryInFlight || !isXrayRunning()) return

        trafficQueryInFlight = true
        Thread {
            try {
                val snapshot = getV2rayTraffic(context)
                if (isXrayRunning()) {
                    lastTrafficSnapshot = snapshot
                }
            } finally {
                trafficQueryInFlight = false
            }
        }.start()
    }
    ...
    fun getV2rayTraffic(context: Context): LongArray {
        ...
        try {
            ...
            if (!process.waitFor(1500, TimeUnit.MILLISECONDS)) {
                ...
                Log.e(TAG, "Stats query timed out")
    ...
    private fun stopTimer() {
        ...
        countDownTimer = null
        seconds = 0
        lastTrafficSnapshot = longArrayOf(0L, 0L, 0L, 0L)
        trafficQueryInFlight = false
    }
    ...
    fun getServerDelay(context: Context, configJson: String, url: String): Long {
        ...
        try {
            ...
            val configFile = File(context.filesDir, "temp_delay_config.json")
            ...
            if (process != null) {
                try {
                    ...
                    Log.e(TAG, "getServerDelay: Failed to stop temp Xray", e)
                    try {
                        process.destroyForcibly()
                    } catch (_: Exception) {}
                }
